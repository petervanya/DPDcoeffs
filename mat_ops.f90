!module mat_ops
!contains
! CANNOT USE module ... contains ... end module STRUCTURE WITH f2py
...
! Create histogram with bins of size dr, version for f2py
    ...
    real(8), intent(in) :: a(n)
    real(8) :: dr
    real(8), intent(out) :: bins(n_b+1)! bins(int(sqrt(real(n)))+2) !(n_bins(a, n, dr)+1)
    integer, intent(out) :: hist(n_b)! hist(int(sqrt(real(n)))+1) !(n_bins(a, n, dr))
    integer :: i, j
    hist = 0
